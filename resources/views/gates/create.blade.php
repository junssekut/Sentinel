<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-semibold text-xl text-navy leading-tight">{{ __('Add Gate') }}</h2>
        </div>
    </x-slot>

    <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white rounded-xl shadow-sm border border-light-200 overflow-hidden">
            <form method="POST" action="{{ route('gates.store') }}" class="p-6 space-y-6">
                @csrf
                <div>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-light-200">
                    ...
                    <button type="submit" class="px-6 py-2 bg-sentinel-blue text-white font-medium rounded-lg hover:bg-sentinel-blue-dark transition-colors">Add Gate</button>
                </div>
            </form>
        </div>
    </div>
